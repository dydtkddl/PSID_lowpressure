...
def train_and_evaluate(X, y, sampler, train_ratio=0.8, qt_frac=0.4,
    ...
    # ── 모델 ──
    model = SimpleMLP(input_dim=X_train.shape[1], hidden1=hidden1, hidden2=hidden2)
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 10 == 0:
            logger.info(f"[Epoch {epoch+1}] Train Loss = {running_loss/len(train_loader):.6f}")
    ...
    model.eval()
    with torch.no_grad():
        y_pred_t = model(X_test_t).squeeze().numpy()
...
def main():
    parser = argparse.ArgumentParser(description="NN training pipeline with sampling strategies")
